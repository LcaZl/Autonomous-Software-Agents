export default class Beliefset {
    ...
    removeAllObjectReferences(obj) {
        if (!(typeof obj === 'string'))
            throw('String expected, got ' + typeof obj + ': ' + obj)
    
        // Iterate over all facts to find and delete those containing the object
        for (let fact of this.#facts.keys()) {
            if (fact.includes(obj)) {
                this.#facts.delete(fact);
            }
        }
    
        // Delete the object itself from the objects set
    ...
    removeFact(fact) {
        ...
        if (this.#facts.has(fact)) {
            ...
            return true;  // Fact removed
        }
        
        return false;  // Fact was not present
    }
